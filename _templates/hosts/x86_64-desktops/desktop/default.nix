...
# ! WARN: If you use Nixos-Anywhere with Disko, you don't need to set Hardware-Configuration.Nix.
# If you're installing on a machine, expect the configuration to not eval while you don't have a hardware-configuration.nix file. You can generate it with `nixos-generate-config` and copy it to this directory.
# Copy the flake from /etc/nixos/ to the path generated by nixos-generate-config, remove the regular configuration, copy the hardware configuration to the host and uncomment this.
{ ... }:
{
  imports = [
